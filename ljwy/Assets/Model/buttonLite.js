...
(function(g, h) {
	if (!g.bShareUtil || !g.bShareControl) {
		...
		var f = g.bShareUtil = {
			...
			isIe6: /msie|MSIE 6/.test(e.userAgent),
			isIe7: /MSIE 7/.test(e.userAgent),
			isIe8: /MSIE 8/.test(e.userAgent),
			isIe9: /MSIE 9/.test(e.userAgent),
			isIe: /Microsoft Internet Explorer/.test(e.appName),
			...
			isTrue: function(a) {
				return typeof a ===
					"boolean" ? a : a.toLowerCase() === "true"
			},
			formatParam: function(a, b) {
				if (a === "number") return parseInt(b, 10);
				else if (a === "boolean") return f.isTrue(b);
				return b
			},
			...
			arrayContains: function(a, b, d) {
				for (var c = a.length; c--;)
					if (!f.isUndefined(b) && a[c] === b) return !0;
					else if (!f.isUndefined(d) && d.test(a[c])) return !0;
				return !1
			},
			loadScript: function(a, b) {
				var d = f.requestedScripts;
				if (!f.arrayContains(d, a)) /(bsMore(Org)?\.js|bshareS887(Org)?\.js)/.test(a) && d.push(a), b = b ||
					function() {}, d = h.createElement("script"), d.src = a, d.type = "text/javascript", d.charset = "utf-8", d.onload = b, d.onreadystatechange = function() {
			...
			getElem: function(a, b, d, c) {
				for (var a = a.getElementsByTagName(b), b = [], j = 0, h = 0, e = a.length; h < e; h++) {
					var g = a[h];
					if (!d || g.className.indexOf(d) !== -1) b.push(g), typeof c === "function" && c(g, j++)
				}
				return b
			},
			getText: function(a) {
			...
			insertAfter: function(a, b) {
				var d = b.parentNode;
				d.lastChild === b ? d.appendChild(a) : d.insertBefore(a,
					b.nextSibling)
			},
			getWH: function() {
				return {
					h: (f.isSt ? k : h.body).clientHeight,
					w: (f.isSt ? k : h.body).clientWidth
				}
			},
			stopProp: function(a) {
				a = a || g.event || {};
			...
			getScript: function(a) {
				for (var b = h.getElementsByTagName("script"), d = [], c = 0, j = b.length; c < j; c++) {
					var e = b[c].src;
			...
			submitForm: function(a, b, d, c) {
				...
				j.action = a;
				for (var e in b)
					if (typeof b[e] !== "function") a = h.createElement("input"), a.type = "hidden", a.name =
						e, a.value = b[e], j.appendChild(a);
				if (f.isIe) h.charset = "utf-8";
				j.submit();
			...
			ready: function(a) {
				if (h.addEventListener) h.addEventListener("DOMContentLoaded", function() {
					...
					h.attachEvent("onreadystatechange", function() {
						h.readyState === "complete" && (h.detachEvent("onreadystatechange",
			...
			createBuzzObject: function(a, b) {
				...
				for (var c in d.defaultConfig) d.config[c] = d.defaultConfig[c];
				d.imageBasePath = d.shost + "/frame/images/";
				d.jsBasePath = d.shost + "/b/";
				d.addEntry = function(a) {
					if (typeof d.counters === "number") d.counters = [];
					d.entries.push(a);
					d.counters.push(0)
				};
				return g[a]
			...
			parseBuzzOptions: function(a, b, d, c, j) {
				var i;
				i = (a = f.getScript(b)[a]) ? f.parseOptions(a.src) : {}, a = i;
				j && (a = j(a));
				for (var e in a) a[e] === void 0 || a[e] === null || typeof d[e] === "number" && a[e] === "" || (d[e] !== void 0 ? d[e] = f.formatParam(typeof d[e], a[e]) : c[e] !== void 0 &&
...
(function(g, h, k) {
	...
	if (!(f.count > 0)) {
		...
		a.loadOptions = function() {
			e.parseBuzzOptions(0, /((bshare|static).*button(Lite)?(Org)?\.js|bshare_load)/, b, d, function(a) {
				if (typeof h.bShareOpt !== "undefined")
					for (var b in h.bShareOpt) a[b] = h.bShareOpt[b];
				...
				if (a.style) a.style = "-1||0||1||2||3||4||5||10||11||999".indexOf(a.style) < 0 ? void 0 : parseInt(a.style, 10);
			...
			a.buttonType = d.type;
			for (var c in a.defaultConfig) a.defaultConfig[c] !==
		...
		a.writeButton = function() {
			...
			f > 1 && f < 6 ? a.writeBshareDiv(c) : f === -1 ? (e.getElem(k, "div", "bshare-custom", function(b) {
			...
			}), (b.beta || b.popjs) && a.writeBshareDiv('<div class="buzzButton">' + b.text + "</div>", "")) : f >= 0 && (f !== 999 && (m = g + "button_custom", f === 0 ? m = g + "logo_square_s.gif" : (m += f + "-" + (b.lang === "en" ? "en" : "zh"), o && (m += "-c"), f === 10 && (n = /(Blue|Red|Green|Grey|Orange)/.test(b.bgc) ? b.bgc : "Orange", m += "-" + n), m += ".gif"), n = d[f][o ? 0 : 1], p = h[f]), c += '<div class="buzzButton bsStyle' + f + '" style="height:' + p + "px;color:" + b.fgc + ";", f ===
			...
			if (!c) {
				...
				var g = 0,
					k = setInterval(function() {
						a.more() ||
							g >= 30 ? clearInterval(k) : ++g
					}, 100);
				return !1
			}
			var i;
			if (c === "bsharesync") i = [a.whost, "/bsyncShare?site=", c].join(""), a.shareStats(c), e.submitForm(i, d, "_blank");
			else if (c === "email") i = [a.bhost, "/bshareEmail"].join(""), a.shareStats(c), e.submitForm(i, d, "_blank");
			else if (c === "clipboard") e.copy2Clipboard(), a.shareStats(c);
			else if (c === "favorite") e.add2Bookmark(), a.shareStats(c);
			else if (c === "printer") e.add2Printer(), a.shareStats(c);
			else {
				i = "";
				if (f.bShareLoad) {
					i = [a.bhost, "/bshare_redirect?site=", c].join("");
					for (var l in d) !/(content|target)/.test(l) &&
						d[l] && typeof d[l] !== "function" && (i += "&" + l + "=" + e.encode(d[l]))
				} else(i = h.BS_PURL_MAP[c]) || alert(a.iL8n.loadFailed), c === "gmw" ? i = e.replaceParam("${URL}", d.url.replace("http://", ""), i) : d.url && (i = e.replaceParam("${URL}", d.url, i)), i = e.replaceParam("${TITLE}", d.title, i), i = e.replaceParam("${CONTENT}", d.summary, i), i = e.replaceParam("${IMG}", d.pic, i), i = e.replaceParam("${VIDEO}", d.video, i);
				h.open(i, "", "height=600,width=800,top=100,left=100,screenX=100,screenY=100,scrollbars=yes,resizable=yes")
			}
		};
		a.onLoad = function() {
			e.getElem(k,
				"a", "bshareDiv",
				function(b, c) {
					e.getElem(b, "div", "buzzButton", function(b) {
			...
			if (b.style === 0) {
				var d = e.getElem(k, "div", "buzzButton")[0].offsetWidth;
				c && (d += 41);
				e.getElem(k, "a", "bshareDiv", function(a) {
					a.style.width = d + "px"
				})
			}
			var f = a.entries.length;
			if (c && f > 0) {
				for (var c = "", g = 0; g < f; g++) {
					var l = a.entries[g];
					if (typeof l.url === "string") {
						if (e.isIe && c.length + l.url.length > 2E3) break;
						c !== "" && (c += "|");
						c += l.url
					}
				}
		...
		a.loadButtonStyle = function() {
			if (a.buttonType !== 15) {
				var c, d = b.style;
				if (b.beta) c = a.jsBasePath + "styles/bshareS888.js?v=20130314";
				else if (b.popjs) c = b.popjs;
				else if (b.style !== -1 && (c = a.jsBasePath + "styles/bshareS" + (d > 1 && d < 6 ? d : 1) + ".js?v=20130314", b.pop === -1 && (d <= 1 || d >= 6))) c = "";
				c && e.loadScript(c)
			}
		};
		a.international = function(c) {
			var d =
				a.jsBasePath + "langs/";
			d += b.lang !== "zh" ? "bs-lang-en.js?v=20130314" : "bs-lang-zh.js?v=20130314";
			e.loadScript(d, c)
		};
		a.start = function() {
			e.loadEngine && (e.loadEngine(g), a.isLite && a.loadOptions(), a.international(function() {
				if (!a.completed) {
					if (b.text === void 0 || b.text === null) b.text = b.style === 0 ? a.iL8n.shareTextShort : a.iL8n.shareText;
					a.isLite && a.renderButton();
					if (a.buttonType === 15) a.boxConfig = {
						position: 0,
						boxHeight: 404,
						boxWidth: 650,
						closeTop: 10,
					...
					b.mdiv >= 0 && a.buttonType !==
						15 && e.loadScript(a.jsBasePath + "components/bsMore.js?v=20130314");
					if (a.buttonType === 1) return a.show(), !1;
					a.loadButtonStyle();
					a.onLoad();
					a.prepare(0);
					a.view(h.location.href);
					setTimeout(function() {
						f.bShareLoad || e.loadScript(a.jsBasePath + "components/bsPlatforms.js?v=20130314")
					}, 3E3);
					a.completed = !0
				}
			}))
		...
		a.init = function() {
			if (!a.isReady) a.isReady = !0, e.loadScript(a.jsBasePath + "engines/bs-engine.js?v=20130314", a.start)
		}
	}
})("bShare", window, document);
(function(g, h, k) {
	if (!(h.bShareControl.count > 0)) {
		...
		f.writeBshareDiv = function(b, d) {
			e.getElem(k, "a", "bshareDiv", function(c) {
				if (b) c.innerHTML = b;
